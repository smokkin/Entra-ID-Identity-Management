...
Actions Taken: Manually created accounts for Edmund Reeve (HR) and Miranda Snider (Operations) via the Entra Admin Center.
...
Metadata Accuracy: Setting specific UsageLocation, Job Title, and Department is not just for bookkeeping. It is critical for Dynamic Groups. For example, by tagging Miranda as "Helpdesk Manager," she can be automatically added to security groups that grant access to ticketing systems via Attribute-Based Access Control (ABAC).

PowerShell Automation
Actions Taken: Installed the Microsoft.Graph module and configured the environment.

Scalability: While manual entry works for two users, it fails for 200. Automation ensures that every user is created with the exact same security posture.
...
Powershell Command: Connect-MgGraph -Scopes "User.ReadWrite.All", "Group.ReadWrite.All"
...
Password Profile Management: We define ForceChangePasswordNextSignIn. In a real-world scenario, setting this to $true ensures the IT admin never knows the user's permanent password, maintaining Zero Trust principles.
